(() => {
    ...
    async function loadLeaderboard() {
        const response = await fetch(`${LEADERBOARD_URL}?t=${Date.now()}`, { cache: 'no-store' });
    ...
    function clearRows() {
        while (leaderboardRows?.firstChild) leaderboardRows.firstChild.remove();
    }
    ...
    async function showLeaderboard() {
        ...
        try {
            ...
            clearRows();
            if (leaderboardStatus) leaderboardStatus.textContent = 'LEADERBOARD TEMPORARILY UNAVAILABLE';
        }
    }
    ...
    function calculateLegendScore() {
        return scoreApi?.calculate(collectedCards, finalReturn) || null;
    }
    ...
    async function prepareScoreSubmission() {
        ...
        try {
            setSubmissionState(score, await loadLeaderboard(), true);
        } catch (error) {
    ...
    function buildSubmissionUrl(score) {
        ...
        const gameLines = score.games
            .map((item) => `- GAME ${item.game}: ${item.asset} | ${item.period} | ${formatPercent(item.excess)} EXCESS`)
            .join('\n');
        const body = [
    ...
    function submitScore() {
        ...
        if (submitStatus) submitStatus.textContent = 'CONFIRM THE PREFILLED ISSUE · ONLY FINAL TOP 10 SCORES ARE STORED';
    }
    ...
    if (typeof champagneBtn !== 'undefined' && champagneBtn) {
        champagneBtn.addEventListener('click', () => requestAnimationFrame(prepareScoreSubmission));
    }
    loadLeaderboard().catch(() => { cachedEntries = []; });
})();
