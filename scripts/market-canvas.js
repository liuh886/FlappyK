(() => {
    ...
    // publishes (run progress, leading/losing) so every skin can reflect the
    // run without opening a new information channel or touching game rules.
    let worldProgress = 0;
    ...
    // with hard-edged rects only, and sit strictly behind plot chrome. The
    // scene is a still image: nothing animates between ticks.
    const STAR_POOL = 96;
    const STAR_FIELDS = 4; // x, y, phase, size
    const starData = new Float32Array(STAR_POOL * STAR_FIELDS);
    const CLOUD_POOL = 3;
    const CLOUD_FIELDS = 3; // x, y, width
    ...
    function drawStarfield(ctx, colors, width, height) {
    ...
    function drawSkyline(ctx, colors, height) {
        ...
        const litThreshold = 0.74 - worldProgress * 0.4;
    ...
    function drawAurora(ctx, colors, width, height) {
        // The aurora's stillness carries the mood: it swells with run progress
        // and burns brighter while leading, dims while the market is ahead.
        ...
        for (let ribbon = 0; ribbon < 3; ribbon += 1) {
            ...
            for (let c = 0; c < columns; c += 1) {
                const x = c * 12;
                const wave =
                    Math.sin(x * 0.016 + ribbon * 2.1) * 10
                    + Math.sin(x * 0.005 + ribbon) * 6;
                const length = (26 + Math.sin(x * 0.011 + ribbon * 1.3) * 14)
                    * (worldLeading === true ? 1.3 : 1);
                ctx.globalAlpha = Math.max(0.015, Math.min(0.3, 0.05 * vigor));
    ...
    function drawIceRidge(ctx, colors, width, height) {
        const baseY = height;
        ctx.save();
    ...
    function drawDunes(ctx, colors, width, height) {
        const layers = [
        ...
        for (let l = 0; l < layers.length; l += 1) {
            ...
            for (let x = -2; x <= width + 2; x += 8) {
                const y = height * layer.y
                    + Math.sin(x / layer.wave * Math.PI * 2 + dunePhases[l]) * layer.amp;
    ...
    function drawBackdrop(ctx, colors, kind, width, height) {
        if (!Number.isFinite(width) || !Number.isFinite(height) || width < 40 || height < 40) return;
        ensureScenery(kind, width, height);

        if (kind === 'polar') {
            drawAurora(ctx, colors, width, height);
            drawIceRidge(ctx, colors, width, height);
        } else if (kind === 'amber') {
            drawPixelSun(ctx, colors, width, height);
            drawDunes(ctx, colors, width, height);
        } else {
            drawStarfield(ctx, colors, width, height);
            drawClouds(ctx, colors, width);
            drawSkyline(ctx, colors, height);
        }
    }
    ...
    function draw(state) {
        ...
        // Skin backdrop scenery sits behind every game object and reflects
        // run progress and the published leading/losing state.
        const skinId = window.FlappyKSkins?.getActive?.() || 'arcade';
        const world = readWorldMood(state);
        worldProgress = world.progress;
        worldLeading = world.leading;
        drawBackdrop(ctx, colors, skinId, width, height);
    ...
    window.FlappyKMarketCanvas = Object.freeze({ draw, refreshPalette });
})();
