...
## Decision Log
- **2026-09-02**: Established dual-kernel philosophy per `PRODUCT_PHILOSOPHY.md` (Market first / Decisions have consequences / Facts before advice / Learning must feel like gameplay, with strict FACT/COUNTERFACTUAL/INTERPRETATION separation and hindsight prohibition). Added `Decision Engine` (`scripts/decision/*` 8 modules: metrics/regime/counterfactual/insight/storage/mastery/recorder/ghost) as a read-only second kernel subscribing to `FlappyKGameController` hooks (`data-resolved/tick/trade/level-did-settle`) and emitting `flappyk:decision-ready` for Ghost Replay + whitelisted verdicts. Blueprint frozen in `docs/DECISION_ENGINE_V01.md`.
