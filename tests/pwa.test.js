...
assert.ok(compactHudStyles.includes("grid-template-columns: minmax(0, 1fr) minmax(108px, 0.86fr)"));
assert.ok(compactHudStyles.includes('width: 176px'));
assert.ok(compactHudStyles.includes('.run-progress-panel .hud-details'));

assert.ok(serviceWorkerSource.includes("const APP_CACHE = 'flappyk-app-v10'"));
assert.ok(serviceWorkerSource.includes("const RUNTIME_CACHE = 'flappyk-runtime-v10'"));
...
console.log('PWA manifest, icons, install UI, analytics, compact modern pixel HUD, reliable cloud sync shell, and refreshed offline cache checks passed');
