(function exposeI18nCore(root, factory) {
    ...
    const zh = {
        ...
        'This friend challenge is invalid.': '该好友挑战无效。',
        'This friend challenge no longer matches the market snapshot.': '该好友挑战已无法匹配当前行情数据快照。',
        ...
        'Market data could not be loaded. Check your connection and try again.': '行情数据加载失败，请检查网络连接后重试。',
        'The result image could not be generated. Please try again.': '无法生成结果图片，请重试。',
    ...
    const zhPatterns = [
        ...
        [/^TODAY (.+%) · DAILY STREAK (\d+)$/, (_, score, streak) => `今日 ${score} · 连续挑战 ${streak} 天`],
        ...
        [/^> CUMULATIVE (.+%)$/, (_, score) => `> 累计收益 ${score}`],
        [/^GAIN (.+%) IN (\d+) DAYS$/, (_, gain, days) => `${days} 天累计收益 ${gain}`],
        [/^EXCESS (.+%) TOTAL$/, (_, excess) => `累计超额 ${excess}`],
